...
namespace MapParse.Types
{
    public class Poly
    {
        ...
        public int NumberOfVertices { get { return Verts.Length; } }
        ...
        public void SplitPoly(Poly poly, ref Poly front, ref Poly back)
        {
            ...
            for (int i = 0; i < poly.NumberOfVertices; i++)
            {
                ...
                if (pointClassification[i] == PointClassification.ON_PLANE &&
                    pointClassification[iNext] != PointClassification.ON_PLANE)
                {
                    ignore = true;
                }
                else if (pointClassification[iNext] == PointClassification.ON_PLANE &&
                         pointClassification[i] != PointClassification.ON_PLANE)
                {
                    ignore = true;
                }
        ...
        public bool CalculatePlane()
        {
            ...
            if (Math.Abs(P.Normal.X) < Constants.Epsilon && Math.Abs(P.Normal.Y) < Constants.Epsilon &&
                Math.Abs(P.Normal.Z) < Constants.Epsilon)
            {
                return false;
            }
